...
class fetch {
	...
	public function __construct() {
		...
		if (! $this->cContent->count ()) {
			 ...
			 * 添加热门职位
			 */
			$keywords = require_once (Path ()->cache.'51job_hotKeywords.php');
			foreach ($keywords as $keyword){
				$this->insert($keyword);
			}
			/*
			//从51采集热门职位
			$this->curl->setUrl('http://search.51job.com/jobsearch/all_hot_keyword.php');
			$content = $this->curl->fetch ();
			$html= str_get_html($content);
			$item = array();
			foreach ( $html->find('table[id=typeSearchTbl0] a') as $element){
				$hot = $element ->plaintext;
				$item[] = $hot;
				//$this->insert($hot);
			}
			$hot_keywords = var_export($item,true);
			file_put_contents(Path ()->cache.'51job_hotKeywords.php', '<?php return '.$hot_keywords.';');
			exit;*/
		}
	}
	//插入待解析的数据
	public function insert($keyword, $page = 1) {
		...
		/*http://search.51job.com/jobsearch/search_result.php?funtype=5300 			兼职
		   http://search.51job.com/jobsearch/search_result.php?funtype=5300&keyword=php&confirmdate=2 三天首发*/
		$fetchTypes = array(
				'兼职'=>'5300',	//兼职
				'实习'=>'1703',	//实习
				'全职'=>'0000'	//全职
		);
		foreach ($fetchTypes as $key => $fetchType){
			$data = array (
					'fetchType' => $key,
					'keyword' => $keyword,
					'page' => $page,
					'url' =>'http://search.51job.com/jobsearch/search_result.php?curr_page=' . $page . '&keyword=' . urlencode ( $keyword ) . '&funtype='.$fetchType.'&confirmdate=2',
					'isFetch' => false,
					'isParse' => false,
...
// 	//采集数据内页入库
	public function run() {
		...
		if ($row) {
			...
			if(mb_strpos($row ['content'],'对不起,没有找到符合您条件的职位') !=false){
				echo 'null data !';
...
class innerParser{
	...
	if ($row) {
		...
		$item['edu'] = empty($education)?null:str_replace(array(" ","　","\t","\n","\r","学&nbsp;&nbsp;&nbsp;&nbsp;历："),array("","","","","",""),$education);
		$item['exp'] = empty($experience)?null:str_replace(array(" ","　","\t","\n","\r","工作年限："),array("","","","","",""),$experience);
		...
		$item['city'] = mb_strpos($arr[1], '-') != false ? mb_substr($arr[1], 0,mb_strpos($arr[1], '-')) :$arr[1];
...
class data {
	...
	public function run() {
		...
		while ( $cursor->hasNext () ) {
			...
			$this->curl = new Spider\Tool\Curl ();
			$this->curl ->setMethod('POST');
			$this->curl ->setUrl('http://spider/index.php?r=spider_job/insert');
			$r = $this->curl->fetch($data);
			echo $r;
